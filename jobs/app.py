...
def review(employer_id):
    ...
    employer = execute_sql('SELECT employer.id, review.title, review.description, review.salary, employer.id as employer_id, employer.name as employer_name FROM review JOIN employer ON employer.id = review.employer_id WHERE review.id = ?', [review_id], single=True)
